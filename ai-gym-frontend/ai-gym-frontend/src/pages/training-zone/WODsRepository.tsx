...
import { Search, Filter, Grid, List, Plus, ChevronDown, Eye, EyeOff, Calendar, Target, Users as UsersIcon, Dumbbell } from 'lucide-react'
...
export function WODsRepository() {
  ...
  const handleCreateNew = useCallback(() => {
    navigate('/training-zone/wods/create')
  }, [navigate])

  const handleWodClick = useCallback((wodId: string) => {
    navigate(`/training-zone/wods/${wodId}/edit`)
  }, [navigate])
              ...
              <>
                {wods.length === 0 ? (
                  <div className="text-center py-20">
                    <div className="mx-auto h-24 w-24 rounded-full flex items-center justify-center mb-4 bg-orange-100">
                    ...
                    <p className="text-sm text-gray-500 mt-2 max-w-sm mx-auto">
                      {filters.search || filters.status !== 'all' || filters.difficulty !== 'all' || filters.communities.length > 0
                        ? `No WODs match your current filters. Try adjusting your search criteria.`
                    ...
                    <div className="mt-8">
                      {filters.search || filters.status !== 'all' || filters.difficulty !== 'all' || filters.communities.length > 0 ? (
                        <button
                          onClick={clearAllFilters}
                          className="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500"
                        >
                          Clear Filters
                        </button>
                      ) : (
                        ...
                        >
                          ...
                          <div className="p-4">
                            ...
                            <div className="flex items-center justify-between mt-3">
                              ...
                              {assignmentCounts[wod.id] > 0 && (
                                <span className="inline-flex items-center text-xs text-gray-500">
                                  <UsersIcon className="h-3 w-3 mr-1" />
                                  {assignmentCounts[wod.id]}
                                </span>
                              )}
                            </div>
                          </div>
                        ...
                        >
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center space-x-3 mt-2">
                                ...
                                {assignmentCounts[wod.id] > 0 && (
                                  <span className="inline-flex items-center text-xs text-gray-500">
                                    <UsersIcon className="h-3 w-3 mr-1" />
                                    {assignmentCounts[wod.id]} assignments
                                  </span>
                                )}
                              </div>
                            </div>
                            <div className="text-right flex-shrink-0">
                              ...
                              <p className="text-sm font-medium text-gray-900">{formatDate(wod.updated_at)}</p>
                            </div>
                          </div>
                        </div>
                      )
