...
export function PageBuilder() {
    ...
    const targetRepository = searchParams.get('repo') || 'wods';
    ...
    const [repositoryPopup, setRepositoryPopup] = useState({ type: '', isOpen: false });
    ...
    const handleLeftMenuToggle = (menuId) => {
    ...
    const addBlockToCurrentPage = (block) => {
        setPageData(prev => ({
            ...prev,
            pages: prev.pages.map(page => page.id === currentPageId
    ...
    const handleBlockUpdate = (updatedBlock) => {
        setPageData(prev => ({
            ...
            pages: prev.pages.map(page => ({
                ...page,
                blocks: page.blocks.map(block => block.id === updatedBlock.id ? updatedBlock : block)
            }))
        }));
        setSelectedBlock(updatedBlock);
    };
    const handleBlockReorder = (blockId, direction) => {
    ...
    const handleOpenRepository = (contentType) => {
    ...
    if (loading) {
        return (_jsx(Layout, { children: _jsx("div", { className: "flex items-center justify-center h-64", children: _jsxs("div", { className: "text-center", children: [_jsx(LoadingSpinner, { size: "lg" }), _jsx("p", { className: "text-sm text-gray-500 mt-3", children: "Loading Page Builder..." })] }) }) }));
    }
    return (_jsxs("div", { className: "h-screen bg-gray-50 flex overflow-hidden", children: [_jsx(FixedLeftRail, { activeMenu: activeLeftMenu, onMenuToggle: handleLeftMenuToggle }), activeLeftMenu && (_jsxs(_Fragment, { children: [console.log('Rendering DeployedLeftMenu with menuType:', activeLeftMenu), _jsx(DeployedLeftMenu, { menuType: activeLeftMenu, onBlockAdd: handleBlockAdd, onClose: () => setActiveLeftMenu(null), wodData: pageData, onWodDataUpdate: setPageData })] })), _jsx("div", { className: "flex-1 flex flex-col", children: _jsx(CenterCanvas, { wodData: pageData, currentPageId: currentPageId, onPageChange: setCurrentPageId, selectedBlock: selectedBlock, onBlockSelect: handleBlockSelect, onBlockReorder: handleBlockReorder, onBlockDelete: handleBlockDelete, onBackToRepository: handleBackToRepository, onSave: savePageData, saving: saving, targetRepository: targetRepository, onRepositoryChange: handleRepositoryChange, error: error, onClearError: () => setError(null), successMessage: successMessage, onClearSuccess: () => setSuccessMessage(null), onPreview: handlePreview }) }), showRightPanel && selectedBlock && (_jsx(RightBlockEditor, { block: selectedBlock, onBlockUpdate: handleBlockUpdate, onClose: () => {
                    setShowRightPanel(false);
                    setSelectedBlock(null);
                }, onOpenRepository: handleOpenRepository })), repositoryPopup.isOpen && (_jsx(RepositoryPopup, { contentType: repositoryPopup.type, onContentSelect: handleContentSelect, onClose: () => setRepositoryPopup({ type: '', isOpen: false }) })), _jsx(PreviewModal, { pageData: pageData, currentPageId: currentPageId, isOpen: showPreview, onClose: () => setShowPreview(false) }), _jsx("div", { className: "hidden", children: _jsxs("select", { value: targetRepository, onChange: (e) => {
                        const newRepo = e.target.value;
                        const params = new URLSearchParams(searchParams);
                        params.set('repo', newRepo);
                        navigate(`/page-builder?${params.toString()}`);
                    }, children: [_jsx("option", { value: "wods", children: "Save to WODs" }), _jsx("option", { value: "blocks", children: "Save to BLOCKS" }), _jsx("option", { value: "programs", children: "Save to PROGRAMS" })] }) })] }));
}
