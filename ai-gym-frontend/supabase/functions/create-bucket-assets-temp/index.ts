...
Deno.serve(async (req) => {
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Upload for assets" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'assets');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for assets" ON storage.objects FOR UPDATE USING (bucket_id = 'assets');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for assets" ON storage.objects FOR DELETE USING (bucket_id = 'assets');`
    ];
