...
import { supabase, ContentItem, ContentType, Client } from '@/lib/supabase'
...
export function ContentRepository({ contentType, title, description, icon: Icon, color }: ContentRepositoryProps) {
  ...
  const filterKey = useMemo(() => {
    return JSON.stringify({
      ...
      clients: filters.clients.sort(),
      status: filters.status,
      sortBy: filters.sortBy,
      sortOrder: filters.sortOrder
    })
  }, [contentType, filters.search, filters.clients, filters.status, filters.sortBy, filters.sortOrder])
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (filteredItems.length > 0) {
        ...
        const [clientAssignments, userAssignments] = await Promise.all([
          supabase.from('content_client_assignments').select('content_item_id').in('content_item_id', itemIds),
          supabase.from('content_user_assignments').select('content_item_id').in('content_item_id', itemIds)
        ])

        const counts: {[key: string]: number} = {}
        itemIds.forEach(id => {
          const clientCount = clientAssignments.data?.filter(a => a.content_item_id === id).length || 0
          const userCount = userAssignments.data?.filter(a => a.content_item_id === id).length || 0
          counts[id] = clientCount + userCount
        })
  ...
  }, [contentType, filterKey, filters.search, filters.clients, filters.status, filters.sortBy, filters.sortOrder])
  ...
  const fetchClients = useCallback(async () => {
    try {
      ...
      } else if (clientsData && mountedRef.current) {
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-4 mt-2">
                    ...
                    {filters.clients.length > 0 && (
                      <span className="text-xs text-gray-500">• {filters.clients.length} client(s)</span>
                    )}
                  </div>
                </div>
              </div>
                ...
                {contentItems.length === 0 ? (
                  <div className="text-center py-20">
                    ...
                    <p className="text-sm text-gray-500 mt-2 max-w-sm mx-auto">
                      {filters.search || filters.status !== 'all' || filters.clients.length > 0
