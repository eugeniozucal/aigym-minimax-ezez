...
const ClientModal: React.FC<ClientModalProps> = ({
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    logoFile: null,
    logoPreview: editingClient?.logo_url || '',
    brandColor: editingClient?.brand_color || '#3B82F6',
    apiKeyId: editingClient?.api_key_id || '',
    forumEnabled: editingClient?.forum_enabled ?? true,
    ...
    try {
      ...
      if (editingClient) {
        ...
        if (formData.startFromTemplate && formData.templateId) {
          // Clone from template
          const { data, error } = await supabase.functions.invoke('clone-client-template', {
            body: {
      ...
      if (result.error) {
        if (result.error.code === '23505') {
          setErrors({ clientName: 'A client with this name already exists' });
          return;
        }
        throw result.error;
      }
