...
const ClientConfig: React.FC = () => {
  ...
  const initializeFeatures = async (clientId: string) => {
    try {
  ...
  const loadTags = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('manage-user-tags', {
        body: { client_id: clientId, action: 'list' }
      });
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      setClient(prev => prev ? { ...prev, ...settingsForm } : null);
        ...
        return (
          <div className="space-y-4">
            <p className="text-gray-600 text-sm mb-6">
              Control which platform features are available to this client's users.
            </p>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  {client.project_name && (
                    <p className="text-gray-600">{client.project_name}</p>
                  )}
                </div>
              </div>
            </div>
