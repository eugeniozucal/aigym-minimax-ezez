...
interface ErrorState {
  message: string
  type: 'network' | 'auth' | 'data' | 'timeout' | 'unknown'
  retryable: boolean
  retryCount: number
}

// Enhanced error handling utility
const createErrorState = (error: any, retryCount = 0): ErrorState => {
  ...
  return { message, type, retryable, retryCount }
}
...
export function Dashboard() {
  ...
  // Enhanced analytics fetch function with timeout and retry
  const fetchAnalyticsData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const data = await retryWithBackoff(fetchWithTimeout, 2, 1000)
      ...
      if (mountedRef.current && !signal.aborted) {
        const errorState = createErrorState(err, error?.retryCount || 0)
        setError(errorState)
        ...
        console.error('Fetch key:', fetchKey)
        console.groupEnd()
      }
    } finally {
      ...
      <div className="space-y-8">
        ...
        {error && (
          <div className={`border rounded-lg p-4 flex items-center space-x-3 ${
            ...
            error.type === 'timeout' ? 'bg-orange-50 border-orange-200' :
            'bg-red-50 border-red-200'
          }`}>
            <AlertCircle className={`w-5 h-5 ${
              error.type === 'auth' ? 'text-red-500' :
              error.type === 'network' ? 'text-yellow-500' :
              error.type === 'timeout' ? 'text-orange-500' :
              'text-red-500'
            }`} />
            <div className="flex-1">
              <span className={`${
                error.type === 'auth' ? 'text-red-700' :
                error.type === 'network' ? 'text-yellow-700' :
                error.type === 'timeout' ? 'text-orange-700' :
                'text-red-700'
              }`}>
                {error.message}
              </span>
            ...
            {error.retryable && (
              <button 
                ...
                className={`font-medium ${
                  error.type === 'auth' ? 'text-red-600 hover:text-red-700' :
                  error.type === 'network' ? 'text-yellow-600 hover:text-yellow-700' :
                  error.type === 'timeout' ? 'text-orange-600 hover:text-orange-700' :
